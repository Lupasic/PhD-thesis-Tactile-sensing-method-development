...
\usepackage[font={large},skip=0pt]{subcaption}
        ...
        \begin{subfigure}[b]{0.3\textwidth}
                ...
                \begin{tikzpicture}
                    ...
                    \begin{scope}[
                        ...
                        node[rounded corners=3pt,left,black,fill=white]{\tiny Splash};
                    \end{scope}
                \end{tikzpicture}
                \caption*{Splash}
...
\begin{frame}[t]{Literature review}
    ...
    \only<2>{\centering\alert{New problem was found}}
\end{frame}
...
\framesubtitle{Assumptions}
    \Large
    \begin{itemize}
        \item Generated terrain family with the same constants will have the same complexity. Parameters:
        \begin{itemize}
            \item cage width and length
            \item cage height range
...
\begin{frame}[t]{Robot design}
    ...
    \begin{columns}[T,onlytextwidth]
        ...
        \begin{column}{0.49\textwidth}
            ...
            \begin{eqnarray*}
                % \resizebox{0.9\hsize}{!}{
                F \rightarrow max = \beta \left( {\omega}_{1} \cdot \overbrace{\delta}^{\text{Distance}} + {\omega}_{2} \cdot \overbrace{\frac{1}{(\gamma - 1) h_{\text{leg}}sin(x)}}^{\text{Simplified body length}}\right) +\\ \nonumber + (1 - \beta) {\delta}^{{\omega}_{1}} {\left( \frac{1}{(\gamma - 1)h_{\text{leg}}sin(x)}\right)}^{{\omega}_{2}}
                % }
            \end{eqnarray*}
            }
            % \vspace{1pt}
...
\begin{frame}[t]{Force transducer design}
    ...
    \only<2>{\LARGE\begin{alertblock}{Answer}
            \color{gray}
            \centering
            Current/voltage measurements from motors \\
            Vision systems \\ 
            {\color{black} Installing a force sensor on each leg} 
        \end{alertblock}}
\end{frame}
...
\begin{frame}[t]{Force transducer design}
    ...
    \begin{multicols}{2}
        \begin{itemize}
            \alt<1>{}{\color{gray}}
            \item  \textbf{F/T sensors}: too big and expensive for small robots.
...
\begin{frame}[t]{Force transducer design}
    ...
    {\Large
        \begin{itemize}
            ...
            \item Position and force repeatability. \only<2>{\\ \alert{Solved by adding a manipulator and a camera}}
            \item To have an ability to apply force only on a particular part of a sensor. \only<2>{\\ \alert{Solved by adding several end-effectors}}
        \end{itemize}}
...
\begin{frame}[t]{Terrain classification}
    ...
    {\Large
        \begin{itemize}
            \item To have a possibility to install new surfaces and change it quickly. \only<2>{\\ \alert{Solved by quick detachable table}}
            \item Infinite movement. \only<2>{\\ \alert{Solved by creating the 2 DoF mechanism and a S-shape leg}}
            \item Movement part should be the same as on the StriRus \only<2>{\\ \alert{Solved by creating a mount for a leg assembly}}
        \end{itemize}}
    \only<2>{\centering\Large\alert{All requirements are fulfilled}}
\end{frame}
