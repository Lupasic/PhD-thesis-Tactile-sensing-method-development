...
\begin{frame}[t]{Разработка преобразователя силы}
    \framesubtitle{Установка: Использование Aruco маркеров}
    \vspace{-15pt}
...
\begin{frame}[t]{Картографирование с помощью датчиков силы}
    \framesubtitle{}
    \only<1-2>{\large\begin{block}{Вопрос}
        Как создать плотное облако точек, используя разреженные данные ног?
        \end{block}}
    \only<2>{\large\begin{alertblock}{Ответ}
    ...
    \begin{itemize}
        ...
        \item \textit{Натурный эксперимент} (-//-): \begin{itemize}
            \large
              ...
              \end{itemize}
              Это приемлимая точность для такой задачи.
    \end{itemize}
\end{frame}
