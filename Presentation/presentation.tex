...
\subtitle{Студент: Олег Буличев \\ Научный руководитель: Александр Малолетов \\ \ } %% title page.
\author{Олег Буличев}
...
\begin{frame}[t]{Разработка робота}
    ...
    \large
    \textbf{Проблема} --  выбрать движитель. Робот должен:
    \begin{itemize}
        ...
        \item возможнсоть \textit{залезать на большие валуны};
    \end{itemize}
...
\begin{frame}[t]{Разработка робота}
    ...
    \only<1-2>{\large\begin{block}{Вопрос}
            Какое оптимальное количество ного должно быть в таком движителе?
        \end{block}}
    \only<2>{\large\begin{alertblock}{Ответ}
            \centering Робот должен иметь \textbf{8---14 ног}!
        \end{alertblock}}
\end{frame}
...
\begin{frame}[c]{Разработка робота}
    ...
    \begin{figure}[H]
        ...
        \begin{subfigure}[t]{0.32\textwidth}
            ...
            \caption*{Генетический алгоритм \\ (OpenAI-ES алгоритм)}
        \end{subfigure}
        \hfill
    \end{figure}
\end{frame}
    ...
    \begin{columns}[T,onlytextwidth]
        ...
        \begin{column}{0.49\textwidth}
            ...
            {\footnotesize
                \begin{eqnarray*}
                    % \resizebox{0.9\hsize}{!}{
                    F \rightarrow max = \beta \left( {\omega}_{1} \cdot \overbrace{\delta}^{\text{Расстояние}} + {\omega}_{2} \cdot \overbrace{\frac{1}{(\gamma - 1) h_{\text{leg}}sin(\alpha)}}^{\text{Упр. длина корпуса}}\right) +\\ \nonumber + (1 - \beta) {\delta}^{{\omega}_{1}} {\left( \frac{1}{(\gamma - 1)h_{\text{leg}}sin(\alpha)}\right)}^{{\omega}_{2}}
                    % }
                \end{eqnarray*}
            }
            % \vspace{1pt}
...
\begin{frame}[t]{Разработка преобразователя силы}
    ...
    \begin{multicols}{2}
        \begin{itemize}
            ...
            \item {\color{black}\textbf{Пьезорезистивный датчик основанный \alt<1>{на чернилах или полимерах}{\underline{Velostat}}}: дешевый и надежный, но имеет пробелмы с гистерезисом}
