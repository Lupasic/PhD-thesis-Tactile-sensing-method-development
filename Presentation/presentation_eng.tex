...
\begin{frame}[t]{Problem statement}
    ...
    \begin{block}{Problem 1}
        \large
        How to obtain a \underline{useful information} on terrain, \textbf{when we have a SLAM based on lidars, cameras}?
        \vspace{5pt}
...
\begin{frame}[t]{Proposed solutions}
    ...
    \begin{exampleblock}{Problem 1}
        \large
        \textit{Map can be built} \textbf{using tactile sensors} on each leg of the robot and create a \textbf{dense point cloud} using sampling from generated mesh from \textbf{modified 2D Delaunay triangulation}.
        \vspace{5pt}
...
\begin{frame}[t]{Robot design}
    ...
    \begin{table}[H]
        ...
        \multicolumn{1}{p{1.3 cm}}{}  & \multicolumn{1}{p{3.5cm}}{\includegraphics[height=3cm,width=3.5cm,keepaspectratio]{strirus_3.JPG}}  &  \multicolumn{1}{p{3.5cm}}{\includegraphics[height=3cm,width=3.5cm,keepaspectratio]{strirus_4.png}} \\
         ...
         \rowcolor[HTML]{EFEFEF} 
         Connect. joint & --- & Pitch \\
        \makecell[l]{Rel. body-leg \\ angle, deg} & 0 & 0, 15 \\
        \rowcolor[HTML]{EFEFEF} 
         Leg height, mm & 90 & 180 \\
         Features & Larger legs & Largest legs \\
        \rowcolor[HTML]{EFEFEF} 
         Withdraws &  \makecell[l]{-- Still small legs \\ -- 1 segment} & \makecell[l]{ -- } \\
        \end{tabular}
        \end{tiny}
        \end{table}
\end{frame}
...
\begin{frame}[t]{Force transducer design}
    ...
    \begin{multicols}{2}
        \begin{itemize}
            ...
            \item { \alt<1>{}{\color{lightgray}} \textbf{Stain gages}: temperature and humidity drasticaly influence on the results}
        \end{itemize}
    \end{multicols}
    \vspace{-10pt}
    \only<2>{\ }
    ...
    \begin{multicols}{2}
        {\large
            \begin{itemize}
                \item Hysteresis
                \item Nonlinearity
                \item Different values with the same pressure if the area of load is less then the sensor
            \end{itemize}}
            \begin{figure}[H]
                \centering
                 \begin{tikzpicture}
...
\begin{frame}[t]{Force transducer design}
    \framesubtitle{Experimental Setup: Overall view on hardware}
    \vspace{-12pt}
    \begin{center}
        \begin{tikzpicture}
...
\begin{frame}[t]{Experiment Design}
    \framesubtitle{Experimental Setup: Video}
    \vspace{-15pt}
    \begin{figure}[H]
...
\begin{frame}[t]{Force transducer design}
    \framesubtitle{Experimental Setup: Angle error correction by Aruco markers}
    \vspace{-15pt}
...
\begin{frame}[t]{Force transducer design}
    \framesubtitle{Experimental Setup: End-effectors}
    \vspace{-0.9cm}
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.6\textwidth}
            \begin{figure}[H]
...
\begin{frame}[t]{Force transducer design}
    ...
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.6\textwidth}
            {\large
                \begin{enumerate}
                    \item \textbf{Static experiment}. We are applying static force for identifying transducer model.
...
\begin{frame}[t]{Terrain classification}
    ...
    \begin{itemize}
        ...
        \item Terrain classification parameters was chosen:
              ...
              \end{itemize}
        \item The Velostat force transducer has been proven to work
    \end{itemize}
\end{frame}
...
\begin{frame}[t]{Map creation based on tactile data}
    \framesubtitle{}
    \only<1-2>{\large\begin{block}{Question}
            How to create a dense point cloud, using sparse data from legs contact points?
        \end{block}}
    \only<2>{\large\begin{alertblock}{Answer}
            \centering \textit{Create a mesh}, using concave hull 2D Delaunay triangulation using sparse data, \textit{sampling a mesh} and return generated points to navigation
        \end{alertblock}}
\end{frame}
...
\begin{frame}[t]{Map creation based on tactile data}
    \framesubtitle{Result: Simulator, path}
    \vspace{-15pt}
    \begin{figure}[H]
        \begin{subfigure}[t]{0.49\textwidth}
...
\begin{frame}[t]{Map creation based on tactile data}
    \framesubtitle{Result: Generated mesh and point cloud}
    \vspace{-15pt}
    \begin{figure}[H]
        \begin{subfigure}[t]{0.49\textwidth}
...
\begin{frame}[t]{Map creation based on tactile data}
    ...
    \large
    \begin{itemize}
        \item Map can be built using concave hull 2D Delaunay triangulation, where input data are the sparse point cloud of contacts.
        ...
        \item \textit{Real world experiment}: \begin{itemize}
            \large
                  \item Avg. Point cloud comparison RMSE is about 8 cm.
                        %     \item Avg. Mesh comparison RMSE is about 1 cm.
              \end{itemize}
             \textit{ It is appropriate accuracy for such task.}
    \end{itemize}
\end{frame}
...
\begin{frame}[t]{Summary}
    ...
    \begin{figure}[H]
        \begin{subfigure}[t]{0.49\textwidth}
            ...
            \caption*{1. Struct. synthesis solution for cycling robots was proposed}
        \end{subfigure}
        \begin{subfigure}[t]{0.49\textwidth}
