\section{"2" Верификация преобразователя силы}
...
\begin{itemize}
    \item Имея конкретного робота и форму ноги можно решить задачу 2 и исследовать и разработать тактильный датчик.
    \item Изучив, было решено разработать свой пьезорезистивный преобразователь силы на основе Velostat *тык*. Данный материал был выбран из-за его отличного соотношения цена/качество. Так как на робота нужно установить много датчиков на каждую ногу, это является важным фактором.
    \item при изучения данного материала я заметил, что когда площадь приложения силы меньше, чем площадь активной части сенсора, то при одинаковом давлении, показания будут разными. А это будет сильно влиять на работу алгоритмов. Примером является маленький камушек, на который робот наехал при ходьбе.
...
\note{\small \setlength{\parindent}{20pt}
\begin{itemize}
    \item Для этого было разработано 2 эксперимента. Статический и динамический.
    \item Первый для калибровки
    \item Второй - основной.
    \item  С помощью эталонного датчика силы, на который была прикреплена насадка *тык*, обеспечивающую конкретную площадь контакта, проводилась касание каждой области сетки. 
    \item Потом значения с эталонного датчика и исследуемого снимались.
\end{itemize}
...
\note{\small \setlength{\parindent}{20pt}
\begin{itemize}
    \item Для калибровки использовался робастный метод наименьших квадратов, экспериментальная установка справа *тык*, формула для регрессии --- слева *тык*. Она основана на знании того, что у нас вязко-эластичный материал, обладающий гистерезисом. Поэтому необходимо учитывать время контакта с поверхностью.
    \item Как можно заметить по графику справа, эта формула подходит для калибровки.
\end{itemize}
}
...
\note{\small \setlength{\parindent}{20pt}
\begin{itemize}
    \item  то было решено создать автоматизированную робототехническую установку *тык*. 
    \item Манипулятор необходим, так данная установка позволяет проводить не только контактные эксперименты, но также и прокатывать валик по сенсору, что ближе к кинематике движения робота. Более того, мы можем проверять и калибровать сенсор уже установленный на ногу робота.
\end{itemize}
...
\note{\small \setlength{\parindent}{20pt}
\begin{itemize}
    \item Результатом изысканий получилось, что можно использовать сенсор, когда площадь нажатия превышает 25 площади датчика.
    \item На слайде можно увидеть 3д гистограмму *тык*, где представлена нормализованная разница между эталонным датчиком и исследуемым для каждого сегмента датчика. Слева при использования насадки в 2 мм диаметром, а справа при 8 мм.
\end{itemize}
}