...
Так же, сделав предположение про то, что робот движется по поверхности, где координата $z$ уникальная для каждого набора $(x,y)$. Возможно реализовать следующий алгоритм. Используя полученное разреженное облако точек, делаем очистку данных. Потом из него создаем полигональную сетку с помощью 2D Триангуляции Делоне \pic{fig:delone_idea.png} (вогнутая оболочка \pic{fig:exp_concave_hull}), а потом генерируем необходимое плотное облако точек. 
...
Эксперимент проводился как в симуляции (Рис. \ref{fig:unsolvable_case}, \ref{fig:start_end_exp}), так и на реальном роботе \pic{fig:real_exp_map_creation}.
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Объяснение необходимости модификации алгоритма}
...
Все это было достигнуто благодаря разборному экспериментальному столу и 2ух степенному механизму, который ходит по окружности \pic{fig:s_shape_leg/s_leg_setup.JPG}.
...
Для начала было решено взять 2 сильно разных поверхности и посмотреть на сырые данные. Было решено использовать резину \pic{fig:s_shape_leg/flat.jpg} и камень \pic{fig:s_shape_leg/view.jpg}.
...
Ниже \pic{fig:data_from_legs} представлены сырые данные с лапок робота. Из-за большого гистерезиса и трудностью с калибровкой необходимо работать с относительными данными.
